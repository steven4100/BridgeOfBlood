...
/// <summary>
/// Uniform grid spatial partition. Build() buckets enemy indices by cell (enemy storage order unchanged).
/// </summary>
public class GridSpatialPartition : IDebugDrawable
{
    ...
    private NativeArray<int> _sortedEnemyIndices;
    private NativeArray<int> _cellIndexByEnemy;
    ...
    public GridSpatialPartition(float2 boundsMin, float2 boundsMax, float cellSize, int maxEnemyCount)
    {
        ...
        _boundsMaxX = boundsMax.x;
        _boundsMaxY = boundsMax.y;
        ...
        _sortedEnemyIndices = new NativeArray<int>(_maxEnemyCount, Allocator.Persistent);
        _cellIndexByEnemy = new NativeArray<int>(_maxEnemyCount, Allocator.Persistent);
    }
    ...
    public void BuildFromPositions(NativeArray<EnemyMotion> motion)
    {
        ...
        new GridSpatialPartitionSerialBuildJob
        {
            ...
            SortedEnemyIndices = _sortedEnemyIndices,
            CellIndexByEnemy = _cellIndexByEnemy
        }.Schedule().Complete();
    }

    /// <summary>
    /// Appends enemy indices in cells overlapping the radius. Indices index into enemy column arrays.
    /// </summary>
        ...
        for (int cy = cellMinY; cy <= cellMaxY; cy++)
        {
            for (int cx = cellMinX; cx <= cellMaxX; cx++)
            {
                ...
                for (int j = start; j < end; j++)
                    results.Add(_sortedEnemyIndices[j]);
            }
        }
    }
    ...
    public void Dispose()
    {
        ...
        if (_sortedEnemyIndices.IsCreated) _sortedEnemyIndices.Dispose();
        if (_cellIndexByEnemy.IsCreated) _cellIndexByEnemy.Dispose();
    }
