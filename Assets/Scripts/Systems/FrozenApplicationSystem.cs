...
[BurstCompile]
public struct FrozenDecideJob : IJobParallelFor
{
    ...
    public void Execute(int i)
    {
        ...
        FrozenApplierRuntime applier = Appliers[hit.attackEntityIndex];
        if (!applier.isActive)
        {
        ...
        {
            Decisions[i] = StatusAilmentFlag.Frozen;
            return;
        }

        var rng = Unity.Mathematics.Random.CreateFromIndex(Seed + (uint)i);
        Decisions[i] = rng.NextFloat() < applier.applyChance ? StatusAilmentFlag.Frozen : StatusAilmentFlag.None;
    }
}

public class FrozenApplicationSystem
{
        ...
        NativeList<StatusAilmentAppliedEvent> ailmentEvents)
    {
        for (int i = 0; i < hitEvents.Length; i++)
        {
            ...
            enemy.statusAilmentFlag |= StatusAilmentFlag.Frozen;
